...
InputCapture_TIM::InputCapture_TIM(TIM_TypeDef *TIMx, u16 minHz, bool enCh1, bool enCh2, bool enCh3, bool enCh4, u8 intGroup, u8 prePriority, u8 subPriority)
{
	...
	if (TIMx == TIM1)      //Timer 1	
	{
		...
		_ch1PinSource = GPIO_PinSource8;    //ch1 gpio pinSource
		_ch2PinSource = GPIO_PinSource9;    //ch2 gpio pinSource
		_ch3PinSource = GPIO_PinSource10;   //ch3 gpio pinSource
		_ch4PinSource = GPIO_PinSource11;   //ch4 gpio pinSource
		_IRQn = TIM1_CC_IRQn;               //timer IRQ channel
		_timer_rcc = RCC_APB2Periph_TIM1;   //clock for timer
		_gpio_rcc = RCC_AHB1Periph_GPIOA;   //clock for gpio
	...
	{
		_port = _port2 = GPIOA;            //gpio port 
		_GPIOAF = GPIO_AF_TIM2;						 //gpio AF
		_ch1Pin = GPIO_Pin_15;             //gpio pin for ch1
		_ch2Pin = GPIO_Pin_1;              //gpio pin for ch2
		_ch3Pin = GPIO_Pin_2;              //gpio pin for ch3
		_ch4Pin = GPIO_Pin_3;              //gpio pin for ch4
		_ch1PinSource = GPIO_PinSource15;  //ch1 gpio pinSource
		_ch2PinSource = GPIO_PinSource1;   //ch2 gpio pinSource
		_ch3PinSource = GPIO_PinSource2;   //ch3 gpio pinSource
		_ch4PinSource = GPIO_PinSource3;   //ch4 gpio pinSource
	...
	else if (_TIMx == TIM3)//Timer 3
	{
		...
		_ch1PinSource = GPIO_PinSource6;   //ch1 gpio pinSource
		_ch2PinSource = GPIO_PinSource7;   //ch2 gpio pinSource
		_ch3PinSource = GPIO_PinSource0;   //ch3 gpio pinSource
		_ch4PinSource = GPIO_PinSource1;   //ch4 gpio pinSource
		_IRQn = TIM3_IRQn;                 //timer IRQ channel
		_timer_rcc = RCC_APB1Periph_TIM3;  //clock for timer
		_gpio_rcc = RCC_AHB1Periph_GPIOA | RCC_AHB1Periph_GPIOB;  //clock for gpio		
	...
	else if (_TIMx == TIM4)//Timer 4
	{
		...
		_ch1PinSource = GPIO_PinSource6;   //ch1 gpio pinSource
		_ch2PinSource = GPIO_PinSource7;   //ch2 gpio pinSource
		_ch3PinSource = GPIO_PinSource8;   //ch3 gpio pinSource
		_ch4PinSource = GPIO_PinSource9;   //ch4 gpio pinSource
		_IRQn = TIM4_IRQn;                 //timer IRQ channel
		_timer_rcc = RCC_APB1Periph_TIM4;  //clock for timer
		_gpio_rcc = RCC_AHB1Periph_GPIOB;  //clock for gpio

#ifdef USE_CAPTURE_TIM4
		pICP4 = this;
#endif
	}
	else if (_TIMx == TIM5)//Timer 4
	{
		_port = _port2 = GPIOA;            //gpio port 
		_GPIOAF = GPIO_AF_TIM5;						 //gpio AF
		_ch1Pin = GPIO_Pin_0;              //gpio pin for ch1
		_ch2Pin = GPIO_Pin_1;              //gpio pin for ch2
		_ch3Pin = GPIO_Pin_2;              //gpio pin for ch3
		_ch4Pin = GPIO_Pin_3;              //gpio pin for ch4
		_ch1PinSource = GPIO_PinSource0;   //ch1 gpio pinSource
		_ch2PinSource = GPIO_PinSource1;   //ch2 gpio pinSource
		_ch3PinSource = GPIO_PinSource2;   //ch3 gpio pinSource
		_ch4PinSource = GPIO_PinSource3;   //ch4 gpio pinSource
		_IRQn = TIM5_IRQn;                 //timer IRQ channel
		_timer_rcc = RCC_APB1Periph_TIM5;  //clock for timer
		_gpio_rcc = RCC_AHB1Periph_GPIOA;  //clock for gpio

#ifdef USE_CAPTURE_TIM5
		pICP5 = this;
#endif
	}
	//RCC Initialize
	...
	{
		mTimeFre = 168;
		RCC_APB2PeriphClockCmd(_timer_rcc, ENABLE);
		RCC_AHB1PeriphClockCmd(_gpio_rcc, ENABLE);
	}
	else //normal timer
	{
		mTimeFre = 84;
	...
	if (enCh1)
		GPIO_PinAFConfig(_port, _ch1PinSource, _GPIOAF); //复用
	if (enCh2)
		GPIO_PinAFConfig(_port, _ch2PinSource, _GPIOAF); //复用
	if (enCh3)
		GPIO_PinAFConfig(_port2, _ch3PinSource, _GPIOAF); //复用
	if (enCh4)
		GPIO_PinAFConfig(_port2, _ch4PinSource, _GPIOAF); //复用
	//Timer Initialize
	u32 res = mTimeFre * 1000000 % minHz;
	u32 multi = mTimeFre * 1000000 / minHz;
